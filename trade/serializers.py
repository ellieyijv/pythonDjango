...
class ShoppingCartSerializer(serializers.Serializer):
    ...
    nums = serializers.IntegerField(required=True, min_value=1, error_messages={
        "min_value": "Number can not less than 1",
        "required": "Please choose the goods"
    })
...
class OrdergGoodsSerializer(serializers.ModelSerializer):
    goods = GoodsSerializer(many=False)
    class Meta:
        model = OrderGoods
        fields = "__all__"
...
class OrderDetailsSerializer(serializers.ModelSerializer):
    goods = OrdergGoodsSerializer(many=True)  
    class Meta:
        model = OrderInfo
        fields = "__all__"
